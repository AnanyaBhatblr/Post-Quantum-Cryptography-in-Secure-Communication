...
// Simple XOR encryption using first N bytes of the shared secret
void xor_encrypt_decrypt(const uint8_t *key, const char *input, char *output, size_t len) {
    for (size_t i = 0; i < len; i++) {
        output[i] = input[i] ^ key[i % 32]; // assuming first 32 bytes as key
    }
    output[len] = '\0';
}
...
int main(int argc, char *argv[]) {
    ...
    // Simulated symmetric encryption using XOR
    char *cipher = malloc(msg_len + 1);
    char *decrypted = malloc(msg_len + 1);

    xor_encrypt_decrypt(ss, plaintext, cipher, msg_len);    // encrypt with shared secret
    xor_encrypt_decrypt(ss2, cipher, decrypted, msg_len);   // decrypt with decapsulated secret
    ...
    OQS_KEM_free(kem);
    free(pk); free(sk); free(ct); free(ss); free(ss2); free(cipher); free(decrypted);
