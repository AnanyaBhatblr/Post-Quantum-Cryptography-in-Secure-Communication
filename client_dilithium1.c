...
int main(int argc, char *argv[]) {
    ...
    if (msg_len > MSG_LEN) {
        fprintf(stderr, "Message too long! Max allowed: %d characters.\n", MSG_LEN);
        return 1;
    }
    ...
    // Sign message
    if (OQS_SIG_sign(sig, signature, &sig_len, (uint8_t *)message, msg_len, secret_key) != OQS_SUCCESS) {
    ...
    fwrite(message, 1, MSG_LEN, msgf);  // Still writes fixed 256 bytes
